        ...
        if self.phase == 'train' and probability < self.epsilon:
            ...
            for action in action_space:
                ...
                for ped_state in state.ped_states:
                    ...
                    current_dual_states.append(current_dual_state)
                    batch_dual_states.append(next_dual_state)
                batch_dual_states = torch.cat(batch_dual_states, dim=0)
                outputs = self.model(batch_dual_states)
                min_output, min_index = torch.min(outputs, 0)
                min_state = current_dual_states[min_index]
