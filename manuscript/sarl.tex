...
In this work, we propose a novel pooling module for the reinforcement learning method, aiming to learn a socially compliant navigation policy in the complex scenes. Draw aspiration from \cite{gupta_social_2018,vemula_social_2017}, we first extract the pair-wise interaction features between the navigator and its neighbors and subsequently employ the soft attention to learn the importance of these features. The impact of intra-interaction on each agent is represented by a local occupancy map. Based on a social state reflecting these interactions, the robot is trained to learn a value network, which is used to evaluate the scenarios consisting of a group of neighbors and produce a cooperative navigation policy. An extensive set of experiments shows that the robot controlled by our model can effectively learn to navigate through a crowd in a socially-compliant manner, outperforming the state-of-the-art methods. 
